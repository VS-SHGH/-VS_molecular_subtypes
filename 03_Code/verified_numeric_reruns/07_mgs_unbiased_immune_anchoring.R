# =============================================================================
# Revision: MGS orientation defined by prespecified biology, NOT by subtype labels
#
...
# Then check: do C1/C2/C3 align along this independently oriented axis?
# =============================================================================
...
if (!file.exists(TPM_FILE)) stop("Missing TPM source: ", TPM_FILE)
if (!file.exists(CLINICAL_FILE)) stop("Missing clinical source: ", CLINICAL_FILE)
set.seed(42)

# ---- Load data ----
tpm_raw <- read_excel(TPM_FILE)
...
stopifnot(length(our_samples) == 38L)
...
stopifnot(!anyNA(subtypes), identical(as.integer(table(subtypes)), c(13L, 12L, 13L)))
...
# 4. Orient MGS based on prespecified external biology:
#    - Schwann cell differentiation score → differentiated end (MGS=0)
#    - Immune score → immune-infiltrated end (MGS=1)
# 5. THEN check if C1/C2/C3 align along this independently oriented axis
# =============================================================================
...
# The high-immune end is MGS=1; the axis is not interpreted as a purely
# cell-intrinsic de-differentiation process.
...
if (all(mgs_order$Subtype == c("C1", "C2", "C3"))) {
  ...
  cat("   The C1-C2-C3 ordering is preserved under the prespecified biological orientation.\n")
  cat("   Direction and scale still depend on the prespecified immune/Schwann score definitions.\n")
} else {
  cat(sprintf("\n⚠️  Natural order is %s, NOT C1-C2-C3.\n",
              paste(mgs_order$Subtype, collapse=" < ")))
  cat("   The C1-C2-C3 ordering is not preserved under this biological orientation.\n")
}
